...
module.exports = {
  async up(queryInterface) {
    // 1. Add 'mobile' and 'fullstack' values to enum_tasks_delivery_area
    await queryInterface.sequelize.query(`
      ALTER TYPE enum_tasks_delivery_area ADD VALUE IF NOT EXISTS 'mobile';
      ALTER TYPE enum_tasks_delivery_area ADD VALUE IF NOT EXISTS 'fullstack';
    `);

    // 2. Update the check_task_subtask_invariants() trigger function to accept 'mobile' and 'fullstack'
    await queryInterface.sequelize.query(`
      ...
      BEGIN
        IF NEW.parent_task_id IS NOT NULL THEN
          ...
          IF NEW.delivery_area IS NULL OR NEW.delivery_area::text NOT IN ('frontend', 'backend', 'mobile', 'fullstack', 'qa') THEN
  ...
  async down(queryInterface) {
    // Revert trigger to original 3 areas
    await queryInterface.sequelize.query(`
      CREATE OR REPLACE FUNCTION check_task_subtask_invariants()
      RETURNS TRIGGER AS $$
      DECLARE
