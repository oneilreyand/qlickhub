...
describe('Task policy', () => {
  test('allows owners, admins, and PO to create or mutate any task', () => {
    for (const role of ['owner', 'admin', 'po'] as const) {
      ...
      assert.doesNotThrow(() =>
        assertCanMutateTask(role, qaUserId, { assigneeId: anotherUserId }, { title: 'Updated' }),
      );
    }
  });
    ...
    assert.throws(
      () =>
        assertCanMutateTask(
          'po',
          qaUserId,
          { parentTaskId: 'parent-1', assigneeId: qaUserId, status: 'in_review' },
          { status: 'done' },
        ),
    ...
    assert.throws(
      () =>
        assertCanMutateTask(
          'qa',
          qaUserId,
          { parentTaskId: null, assigneeId: qaUserId },
          { title: 'Test' },
        ),
  ...
  test('allows assigned Dev to follow valid transition map, rejecting invalid jumps and direct done', () => {
    ...
    assert.doesNotThrow(() =>
      assertCanMutateTask(
        'dev',
        qaUserId,
        { parentTaskId: 'parent-1', assigneeId: qaUserId, status: 'todo' },
        { status: 'in_progress' },
      ),
    );
    ...
    assert.doesNotThrow(() =>
      assertCanMutateTask(
        'dev',
        qaUserId,
        { parentTaskId: 'parent-1', assigneeId: qaUserId, status: 'in_progress' },
        { status: 'in_review' },
      ),
    );
    ...
    assert.doesNotThrow(() =>
      assertCanMutateTask(
        'dev',
        qaUserId,
        { parentTaskId: 'parent-1', assigneeId: qaUserId, status: 'changes_requested' },
        { status: 'in_progress' },
      ),
    );
    ...
    assert.throws(
      () =>
        assertCanMutateTask(
          'dev',
          qaUserId,
          { parentTaskId: 'parent-1', assigneeId: qaUserId },
          { title: 'Unpermitted' },
        ),
    ...
    assert.throws(
      () =>
        assertCanMutateTask(
          'dev',
          qaUserId,
          { parentTaskId: 'parent-1', assigneeId: anotherUserId, status: 'todo' },
          { status: 'in_progress' },
        ),
  ...
  test('allows QA to review subtasks in review and execute assigned QA subtasks, rejecting invalid reviews and planning fields', () => {
    ...
    assert.doesNotThrow(() =>
      assertCanMutateTask(
        'qa',
        qaUserId,
        { parentTaskId: 'parent-1', assigneeId: qaUserId, status: 'todo', deliveryArea: 'qa' },
        { status: 'in_progress' },
      ),
    );
    assert.doesNotThrow(() =>
    ...
    assert.throws(
      () =>
        assertCanMutateTask(
          'qa',
          qaUserId,
          { parentTaskId: 'parent-1', assigneeId: anotherUserId, status: 'in_review' },
          { priority: 'urgent' },
        ),
  ...
  test('enforces assigned QA execution lifecycle without self-review', () => {
    const assignedQaTask = (status: 'todo' | 'in_progress' | 'in_review' | 'done') => ({
      parentTaskId: 'parent-1',
      assigneeId: qaUserId,
    ...
    assert.doesNotThrow(() =>
      assertCanMutateTask('qa', qaUserId, assignedQaTask('in_progress'), { status: 'done' }),
    );
    assert.throws(
      () =>
        assertCanMutateTask('qa', qaUserId, assignedQaTask('in_progress'), { status: 'in_review' }),
      /Invalid status transition for QA executor from "in_progress" to "in_review"/,
    );
    assert.throws(
      () => assertCanMutateTask('qa', qaUserId, assignedQaTask('done'), { status: 'in_progress' }),
      /A reason is required when reopening completed QA execution/,
    );
    assert.doesNotThrow(() =>
      assertCanMutateTask('qa', qaUserId, assignedQaTask('done'), {
        status: 'in_progress',
        reviewNotes: 'Regression pass requested for the release candidate.',
      }),
    );
    assert.doesNotThrow(() =>
      assertCanMutateTask('qa', qaUserId, assignedQaTask('in_review'), { status: 'in_progress' }),
    );
    assert.doesNotThrow(() =>
      assertCanMutateTask('qa', qaUserId, assignedQaTask('in_review'), { status: 'done' }),
    );
    assert.throws(
      () =>
        assertCanMutateTask(
          'qa',
          qaUserId,
          { ...assignedQaTask('in_review'), assigneeId: anotherUserId },
          { status: 'done' },
        ),
      /QA members cannot execute QA subtasks assigned to other members/,
    );
  ...
  test('forbids moving subtasks independently', () => {
    assert.throws(
      () => assertCanMoveTask('owner', true),
      /Subtasks cannot be moved independently from their parent task/,
    );
    assert.throws(
      () => assertCanMoveTask('po', true),
      /Subtasks cannot be moved independently from their parent task/,
    );
  });
  ...
  test('enforces role-based task access scoping', () => {
    ...
    assert.doesNotThrow(() =>
      assertCanAccessTask('dev', qaUserId, { id: 'task-3', reporterId: qaUserId }, false),
    );
