...
describe("LIVE E2E DEMONSTRATION SCENARIO (1 PO, 1 DEV, 1 QA)", () => {
  ...
  before(async () => {
    ...
    await WorkspaceMemberModel.create({ workspaceId: workspace.id, userId: poUser.id, role: "owner" });
    await WorkspaceMemberModel.create({ workspaceId: workspace.id, userId: devUser.id, role: "dev" });
  ...
  test("E2E Step 3: PO merencanakan Folder, Parent Task, dan Subtasks di Task Hub", async () => {
    ...
    console.log(`   ✅ Parent Task: "${parentTask.title}" (ID: ${parentTask.id}, Status: ${parentTask.status})`);
  ...
  test("E2E Step 9: Penutupan Parent Task oleh PO di Task Hub", async () => {
    ...
    const completedParent = await taskService.completeTask(poUser.id, workspace.id, parentTask.id, { status: "done" });
    console.log(`   ✅ Parent Task "${completedParent.title}" -> STATUS: ${completedParent.status.toUpperCase()}`);
