...
const TARGET_FIELDS = [
  { key: '', label: '(Abaikan / Jangan Impor)' },
  { key: 'title', label: 'Judul (Wajib)' },
  { key: 'requirement_code', label: 'Kode Requirement (Wajib)' },
  { key: 'external_reference', label: 'Nomor Test Case (opsional, otomatis jika kosong)' },
  { key: 'steps', label: 'Langkah / Prosedur' },
  { key: 'expected_result', label: 'Hasil yang Diharapkan' },
  { key: 'test_data', label: 'Data Pengujian' },
  { key: 'priority', label: 'Prioritas (high, medium, low)' },
  { key: 'scenario_kind', label: 'Jenis Skenario (positive, negative, edge)' },
  { key: 'test_type', label: 'Jenis Pengujian (manual, e2e, integration, unit)' },
  { key: 'preconditions', label: 'Prasyarat' },
];
...
export const TestCaseImportWizardModal: React.FC<TestCaseImportWizardModalProps> = ({
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      setErrorMessage('Template CSV gagal diunduh.');
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : 'Spreadsheet gagal dibaca.');
      setLoading(false);
    }
  };
  ...
  const handleApplyMapping = async () => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : 'Pratinjau simulasi gagal dibuat.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSheetChange = async (newSheet: string) => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : 'Sheet yang dipilih gagal dibaca.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : 'Proses impor gagal disimpan.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadErrorReport = async (importId: string) => {
    try {
      ...
      setErrorMessage('Laporan error gagal diunduh.');
    }
  };
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : 'Riwayat audit gagal dimuat.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal
      ...
      title={
        step === 'history'
          ? 'Riwayat Impor & Log Audit'
          : step === 'result'
            ? 'Impor Selesai'
            : step === 'mapping'
              ? 'Petakan Kolom Spreadsheet'
              : step === 'preview'
                ? 'Pratinjau Simulasi Impor'
                : 'Impor Test Case dari Spreadsheet'
      }
      description="Impor Test Case dari CSV atau XLSX dengan penautan Requirement otomatis dan aman dari duplikasi."
      size="4xl"
    >
          ...
          <div className="space-y-4 max-h-[70vh] flex flex-col">
            ...
            {previewData.availableSheets && previewData.availableSheets.length > 1 && (
              <div className="flex items-center justify-between gap-3 p-3 bg-slate-800/50 rounded-xl border border-slate-700 text-xs">
                ...
                <select
                  aria-label="Pilih sheet spreadsheet"
                  value={selectedSheet}
              ...
              {(previewData.headers || []).map((header) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <select
                        aria-label={`Field tujuan untuk kolom ${header}`}
                        value={currentTarget}
                        onChange={(e) => {
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              >
                {loading ? 'Menganalisis...' : 'Berikutnya: Pratinjau Simulasi'}
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 max-h-[70vh] flex flex-col">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <Button
                ...
                leftIcon={<ArrowRight className="w-4 h-4" />}
              >
                {loading ? 'Mengimpor...' : `Impor ${previewData.validRows} Baris Valid`}
              </Button>
            </div>
          </div>
        )}
