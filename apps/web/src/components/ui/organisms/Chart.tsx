...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  secondaryLabel = 'Pengujian Gagal',
  height = 200,
}) => {
  const [hoveredIdx, setHoveredIdx] = useState<number | null>(null);

  const maxValue = Math.max(...data.map((d) => (d.value || 0) + (d.secondaryValue || 0)), 10);
  ...
  return (
    <div className="w-full rounded-[24px] border border-stone-200/80 bg-white p-5 shadow-xs dark:border-stone-800/80 dark:bg-[#1C1A19] dark:text-stone-100">
      ...
      {/* Chart Bars */}
      <div
        className="relative flex items-end justify-between gap-2 pt-6 px-2"
          ...
          {points.map((p, idx) => (
            <g key={idx} className="group cursor-pointer">
              <circle
                ...
                cy={p.y}
                r="5"
                className="fill-[#B1E743] stroke-stone-900 dark:stroke-stone-100 stroke-2 transition-transform group-hover:r-7"
              />
              <text
                x={p.x}
                y={chartHeight - 10}
                textAnchor="middle"
                className="text-[10px] font-semibold fill-stone-500 dark:fill-stone-400"
              >
                {p.data.label}
              </text>
              <text
                ...
                y={p.y - 12}
                textAnchor="middle"
                className="text-[10px] font-bold fill-stone-900 opacity-0 group-hover:opacity-100 transition-opacity dark:fill-[#B1E743]"
              >
                {p.data.value}%
              </text>
            </g>
