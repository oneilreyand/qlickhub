...
import type { Task, FolderTreeNode, TaskStatus, ParentTaskDeliveryTrace } from '@qlick/contracts';
...
export interface TaskDeliveryTraceRowState {
...
interface TaskCollectionProps {
  ...
  deliveryTraceStateByTaskId?: Record<string, TaskDeliveryTraceRowState>;
  releaseReadinessStateByTaskId?: ReleaseReadinessStateMap;
...
export const TaskCollection: React.FC<TaskCollectionProps> = ({
  ...
  const isAllCollapsed =
    groupedTasks.length > 0 && groupedTasks.every((group) => collapsedStatuses.has(group.status));
                    ...
                    group.tasks.map((task) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <DeliveryTraceSignal
                            trace={deliveryTraceState?.trace}
                            isLoading={!deliveryTraceState || deliveryTraceState.isLoading}
                            error={deliveryTraceState?.error}
                            permissionDenied={deliveryTraceState?.permissionDenied}
                          />
                          {releaseReadinessStateByTaskId && (
              ...
              : groupedTasks.flatMap((group) => {
                  ...
                  return [
                    <tr
                      ...
                      className="bg-stone-50/70 dark:bg-stone-900/70"
                    >
