...
const Section: React.FC<{ category: string; title: string; description: string; children: React.ReactNode }> = ({
  category,
  title,
  description,
  children,
...
export const ComponentGalleryPage: React.FC = () => {
  ...
  const sampleTableData = [
    ...
    { id: 'TASK-102', title: 'JWT Refresh Token revocation', req: 'REQ-04', status: 'Passed', owner: 'Sarah Connor' },
    { id: 'TASK-103', title: 'Stripe Webhook Signature check', req: 'REQ-12', status: 'In Review', owner: 'Alex Smith' },
    { id: 'TASK-104', title: 'Payment Gateway 502 Retry Policy', req: 'REQ-14', status: 'Blocked', owner: 'Mike Ross' },
  ];
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12 animate-fadeIn">
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Section category="Atoms" title="Buttons" description="Primary, secondary, destructive, ghost, and icon button variants with loading states.">
          ...
          <Section category="Atoms" title="Badges" description="Status indicators representing QA states and system metadata tags.">
          ...
          <Section category="Atoms" title="Inputs & Search" description="Form text fields with icons, shortcuts, and validation state.">
          ...
          <Section category="Atoms" title="Avatars" description="User initials badge with status indicators and size variants.">
          ...
          <Section category="Atoms" title="Skeletons" description="Animated placeholders used during API data loading.">
          ...
          <Section category="Atoms" title="Tooltips" description="Contextual micro-popovers with directional positioning (top, bottom, left, right).">
            <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="grid gap-6 md:grid-cols-2">
            ...
            <div>
              <DateRangePicker
                ...
                onChange={(range) => {
                  setDateRange(range);
                  if (range) {
                    dispatch(enqueueSnackbar(`Selected range: ${range.startDate} to ${range.endDate}`, 'info'));
                  }
                }}
              />
            </div>
          ...
          <Section category="Molecules" title="Modal & Drawer Controls" description="Overlay popups for confirmation actions and detail side inspection panels.">
            <div className="flex flex-wrap items-center gap-3">
          ...
          <Section category="Molecules" title="Navigation Tabs" description="Underline and pill navigation tab bars with badge counts.">
            <div className="space-y-4">
              <Tabs
                variant="underline"
          ...
          <Section category="Molecules" title="Dropdown Context Menu" description="Popover action list trigger for record mutations.">
            <DropdownMenu
              ...
              items={[
                ...
                },
                {
                  ...
                  onClick: () => {
                    dispatch(reportError('Delete record requires confirmation in the production workflow.'));
                  },
                },
              ]}
            />
          ...
          <Section category="Molecules" title="Empty & Error State" description="Illustrative container when search yields 0 items or dataset is empty.">
            <EmptyState
          ...
          >
            <div className="space-y-4">
                ...
                <Button
                  ...
                  onClick={() =>
                    dispatch(
                      enqueueApiResponse({
                        status: 403,
                        detail: 'API 403 Forbidden: You lack admin privileges for this workspace action',
                      })
                    )
                  }
                >
              ...
              <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-stone-100 dark:border-stone-800">
                <span className="text-xs font-semibold text-stone-500">Async Thunk API Simulation:</span>
                <Button
                  variant="primary"
                  size="sm"
                  isLoading={isGlobalActionPending}
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="space-y-6">
          ...
          <Section category="Organisms" title="Analytics Charts & Visualizations" description="Bar charts and trend line charts for dashboard test execution metrics.">
            <div className="grid gap-6 md:grid-cols-2">
              <BarChart
          ...
          <Section category="Organisms" title="Dashboard Stat Cards" description="KPI metric overview cards with trend indicators.">
          ...
          <Section category="Organisms" title="Data Table" description="Structured dashboard table with custom cell rendering and pagination.">
                ...
                columns={[
                  {
                    key: 'id',
                    header: 'Task ID',
                    render: (row) => <span className="font-mono font-bold text-[#22201F] dark:text-[#B1E743]">{row.id}</span>,
                  },
                  ...
                  },
                  {
                    ...
                    render: (row) => (
                      <Badge
                        variant={
                          row.status === 'Passed' ? 'passed' : row.status === 'In Review' ? 'review' : 'blocked'
                        }
                      >
                        {row.status}
                      </Badge>
      ...
      >
        <div className="space-y-4">
          <Input label="Requirement Title" placeholder="e.g., REQ-25: Multi-Factor Authentication" />
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-stone-700 dark:text-stone-300">Acceptance Criteria</label>
            <textarea
              rows={3}
      ...
      >
        <div className="space-y-4">
          <div className="rounded-xl border border-stone-200 bg-stone-50 p-4 dark:border-stone-800 dark:bg-stone-900">
            <h4 className="text-xs font-bold text-stone-900 dark:text-stone-100">Linked Requirement: REQ-01</h4>
            <p className="mt-1 text-xs text-stone-600 dark:text-stone-400">OAuth2 Multi-provider authentication flow with Google & GitHub providers.</p>
          </div>
          <div className="space-y-2">
            <h4 className="text-xs font-bold text-stone-800 dark:text-stone-200">QA Readiness Signal</h4>
