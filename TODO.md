...
- [x] **Done** (Antigravity — 2026-08-24) — **QA-REVIEW-REMEDIATION: Remediate review findings for Test Case intake authorization, import session integrity & tamper protection, HTTPS-only evidence security, accessible modal with zoom/sandboxed iframe, unified attachment & external evidence UI, and spreadsheet sheet selection/header mapping.** Addressed review blockers P1–P6 per ADR-001, Atomic Design policy, and verified PostgreSQL integration tests. Strict QA update authorization enforced on native and import paths; tamper-proof server-side staged sessions with content hash and expiration; strict HTTPS-only validation and in-memory parsing without SSRF; accessible EvidencePreviewModal with shared Modal, keyboard zoom, and ARIA labels; registered in ComponentGalleryPage; unified formal attachments and evidence links with provenance tags; multi-sheet XLSX tab discovery and selection; 500-row limits. Evidence: `packages/contracts` (54/54 pass), `apps/api` (250/250 tests across 69 suites pass, clean migration verified), `apps/web` (59 files, 269/269 pass, build exit 0), and clean `npm run lint`.
