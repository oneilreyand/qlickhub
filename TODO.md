...
- [x] **Done** (Codex — 2026-08-21) — **AGY-0.1: Reconcile repository, migration, and database state.** Inspect migrations 47 and 48, the live development schema, model/contract enums, and the attachment-removal conflict with the delivery plan. The legacy form of migration 48 dropped data and therefore required a new additive recovery migration instead of rewriting an already-recorded migration. Resolve migration drift using additive/recoverable changes where possible. **Acceptance:** clean-database migration succeeds; development migration status matches the approved schema; Mobile/Fullstack persistence is proven against PostgreSQL; no user data is silently deleted.
- [x] **Done** (Codex — 2026-08-21) — **AGY-0.2: Restore a green policy and contract baseline.** Reconcile task policy implementation, role expectations, QA document types, and stale tests before feature work. **Acceptance:** applicable contract and backend policy suites pass; every changed permission has explicit Owner/PO/Dev/QA cases; failures are fixed rather than skipped or weakened.
- [ ] **Blocked** (owner decision required — 2026-08-21) — **AGY-0.3: Record unresolved domain decisions.** Confirm who may create Dev/QA work, who may transition executor status, whether Requirement or Acceptance Criterion is the atomic coverage target, and whether evidence attachments remain part of the product. **Acceptance:** decisions are documented in the applicable plan or ADR; no implementation begins from an unstated assumption.
